...
let NotaryService = class NotaryService {
    ...
    async getAllNotary(pagination) {
        const pattern = { cmd: 'getAllNotary' };
        return await this.clientProxy.send(pattern, pagination);
    }
    async getNotaryById(id) {
        const pattern = { cmd: 'getNotaryById' };
        return await this.clientProxy.send(pattern, id);
    }
    async createNotary(notaryDTO) {
        const pattern = { cmd: 'createNotary' };
        return await this.clientProxy.send(pattern, notaryDTO);
    }
    async updateNotary(notaryDTO, id) {
        notaryDTO.id = id;
        const pattern = { cmd: 'updateNotary' };
        return await this.clientProxy.send(pattern, notaryDTO);
    }
    async deleteNotary(id) {
        const pattern = { cmd: 'deleteNotary' };
