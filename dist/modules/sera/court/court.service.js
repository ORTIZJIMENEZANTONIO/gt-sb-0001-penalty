...
let CourtService = class CourtService {
    ...
    async getAllCourt(pagination) {
        const pattern = { cmd: 'getAllCourt' };
        return await this.clientProxy.send(pattern, pagination);
    }
    async getCourtById(id) {
        const pattern = { cmd: 'getCourtById' };
        return await this.clientProxy.send(pattern, id);
    }
    async createCourt(courtDTO) {
        const pattern = { cmd: 'createCourt' };
        return await this.clientProxy.send(pattern, courtDTO);
    }
    async updateCourt(courtDTO, id) {
        courtDTO.id = id;
        const pattern = { cmd: 'updateCourt' };
        return await this.clientProxy.send(pattern, courtDTO);
    }
    async deleteCourt(id) {
        const pattern = { cmd: 'deleteCourt' };
