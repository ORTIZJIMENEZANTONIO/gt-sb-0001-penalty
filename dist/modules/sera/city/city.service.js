...
let CityService = class CityService {
    ...
    async getAllCity(pagination) {
        const pattern = { cmd: 'getAllCitySera' };
        return await this.clientProxy.send(pattern, pagination);
    }
    async getCityById(id) {
        const pattern = { cmd: 'getCityBySeraId' };
        return await this.clientProxy.send(pattern, id);
    }
    async createCity(cityDTO) {
        const pattern = { cmd: 'createCitySera' };
        return await this.clientProxy.send(pattern, cityDTO);
    }
    async updateCity(cityDTO, id) {
        cityDTO.id = id;
        const pattern = { cmd: 'updateCitySera' };
        return await this.clientProxy.send(pattern, cityDTO);
    }
    async deleteCity(id) {
        const pattern = { cmd: 'deleteCitySera' };
