...
let PersonService = class PersonService {
    ...
    async getAllPerson(pagination) {
        const pattern = { cmd: 'getAllPerson' };
        return await this.clientProxy.send(pattern, pagination);
    }
    async getPersonById(id) {
        const pattern = { cmd: 'getPersonById' };
        return await this.clientProxy.send(pattern, id);
    }
    async createPerson(personDTO) {
        const pattern = { cmd: 'createPerson' };
        return await this.clientProxy.send(pattern, personDTO);
    }
    async updatePerson(personDTO, id) {
        personDTO.id = id;
        const pattern = { cmd: 'updatePerson' };
        return await this.clientProxy.send(pattern, personDTO);
    }
    async deletePerson(id) {
        const pattern = { cmd: 'deletePerson' };
