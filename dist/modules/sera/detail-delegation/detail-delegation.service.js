...
let DetailDelegationService = class DetailDelegationService {
    ...
    async getAllDetailDelegation(pagination) {
        const pattern = { cmd: 'getAllDetailDelegation' };
        return await this.clientProxy.send(pattern, pagination);
    }
    async getDetailDelegationById(id) {
        const pattern = { cmd: 'getDetailDelegationById' };
        return await this.clientProxy.send(pattern, id);
    }
    async createDetailDelegation(detailDelegationDTO) {
        const pattern = { cmd: 'createDetailDelegation' };
        return await this.clientProxy.send(pattern, detailDelegationDTO);
    }
    async updateDetailDelegation(detailDelegationDTO, id) {
        detailDelegationDTO.id = id;
        const pattern = { cmd: 'updateDetailDelegation' };
        return await this.clientProxy.send(pattern, detailDelegationDTO);
    }
    async deleteDetailDelegation(id) {
        const pattern = { cmd: 'deleteDetailDelegation' };
