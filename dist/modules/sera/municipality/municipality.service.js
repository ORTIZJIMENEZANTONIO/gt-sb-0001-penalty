...
let MunicipalityService = class MunicipalityService {
    ...
    async getAllMunicipality(pagination) {
        const pattern = { cmd: 'getAllMunicipalitySera' };
        return await this.clientProxy.send(pattern, pagination);
    }
    async getMunicipalityById(id) {
        const pattern = { cmd: 'getMunicipalitySeraById' };
        return await this.clientProxy.send(pattern, id);
    }
    async createMunicipality(municipalityDTO) {
        const pattern = { cmd: 'createMunicipalitySera' };
        return await this.clientProxy.send(pattern, municipalityDTO);
    }
    async updateMunicipality(municipalityDTO, id) {
        municipalityDTO.id = id;
        const pattern = { cmd: 'updateMunicipalitySera' };
        return await this.clientProxy.send(pattern, municipalityDTO);
    }
    async deleteMunicipality(id) {
        const pattern = { cmd: 'deleteMunicipalitySera' };
