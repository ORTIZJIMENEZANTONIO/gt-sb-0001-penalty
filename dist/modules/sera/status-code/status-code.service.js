...
let StatusCodeService = class StatusCodeService {
    ...
    async getAllStatusCode(pagination) {
        const pattern = { cmd: 'getAllStatusCode' };
        return await this.clientProxy.send(pattern, pagination);
    }
    async getStatusCodeById(id) {
        const pattern = { cmd: 'getStatusCodeById' };
        return await this.clientProxy.send(pattern, id);
    }
    async createStatusCode(statusCodeDTO) {
        const pattern = { cmd: 'createStatusCode' };
        return await this.clientProxy.send(pattern, statusCodeDTO);
    }
    async updateStatusCode(statusCodeDTO, id) {
        statusCodeDTO.id = id;
        const pattern = { cmd: 'updateStatusCode' };
        return await this.clientProxy.send(pattern, statusCodeDTO);
    }
    async deleteStatusCode(id) {
        const pattern = { cmd: 'deleteStatusCode' };
        return await this.clientProxy.send(pattern, id);
    }
};
