...
let RegulatoryService = class RegulatoryService {
    ...
    async getAllRegulatory(pagination) {
        const pattern = { cmd: 'getAllRegulatory' };
        return await this.clientProxy.send(pattern, pagination);
    }
    async getRegulatoryById(id) {
        const pattern = { cmd: 'getRegulatoryById' };
        return await this.clientProxy.send(pattern, id);
    }
    async createRegulatory(regulatoryDTO) {
        const pattern = { cmd: 'createRegulatory' };
        return await this.clientProxy.send(pattern, regulatoryDTO);
    }
    async updateRegulatory(regulatoryDTO, id) {
        regulatoryDTO.id = parseInt(`${id}`);
        const pattern = { cmd: 'updateRegulatory' };
        return await this.clientProxy.send(pattern, regulatoryDTO);
    }
    async deleteRegulatory(id) {
        const pattern = { cmd: 'deleteRegulatory' };
